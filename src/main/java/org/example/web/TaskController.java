...
import org.example.service.TaksService;
...
public class TaskController {
    ...
    public ResponseEntity<Task> createTask(
        ...
        var result = this.taksService.add(req.getTitle(), req.getDescription(), req.getDueDate());
    ...
    public ResponseEntity<List<Task>> getTask(Optional<String> dueDate) {
        ...
        if(dueDate.isPresent()) {
            result = this.taksService.getByDueDate(dueDate.get()); // 해당 마감일 조회
        } else {
            result = this.taksService.getAll(); // 모든 데이터 조회
        }
    ...
    public ResponseEntity<Task> fetchOneTask(
        ...
        var result = this.taksService.getOne(id);
        return ResponseEntity.ok(result);
    }
    ...
    public ResponseEntity<List<Task>> getByStatus(
        ...
        var result = this.taksService.getByStatus(status);
        return ResponseEntity.ok(result);
    }
}
